    ...
    <div className="space-y-6">
        ...
        <Card className="p-4">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500">
              Successful ({formatPercentage((metrics.totals.count - metrics.totals.errors) / metrics.totals.count)})
            </div>
          </div>
        </Card>
        ...
        <div className="space-y-4">
          <div>
            <div className="text-xl font-semibold">
              {formatPercentage(metrics.confidence.mean)}
            </div>
      ...
      <Card className="p-6 bg-blue-50 border-blue-200 text-black">
        ...
        <ul className="space-y-2 text-sm text-blue-700">
          <li>
            • <strong>Success Rate:</strong> {formatPercentage((metrics.totals.count - metrics.totals.errors) / metrics.totals.count)} 
            of transactions were successfully categorized
          </li>
          <li>
            • <strong>Engine Efficiency:</strong> {formatPercentage(metrics.totals.pass1Only / metrics.totals.count)} 
            were handled by fast Pass-1 rules alone
          </li>
          <li>
            • <strong>Performance:</strong> Average {formatLatency(metrics.latency.mean)} per transaction 
            (P95: {formatLatency(metrics.latency.p95)})
          </li>
          {metrics.accuracy && (
            <li>
              • <strong>Accuracy:</strong> {formatPercentage(metrics.accuracy.overall)} overall accuracy 
              based on ground truth data
            </li>
          )}
          {metrics.cost && (
            <li>
              • <strong>Cost:</strong> ${metrics.cost.estimatedUsd.toFixed(4)} estimated for {metrics.cost.calls} LLM calls
            </li>
          )}
        </ul>
      </Card>
