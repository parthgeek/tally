...
/**
 * Create a safe version of response data for logging
 */
export function safeResponseLog(response: Response): any {
  return {
    status: response.status,
    statusText: response.statusText,
    headers: redactSensitiveFields(Object.fromEntries(response.headers.entries())),
    // Never log the body - it may contain sensitive data
