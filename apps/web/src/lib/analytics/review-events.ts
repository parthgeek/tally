...
export const reviewEvents = {
  ...
  transactionCorrected: async (
    ...
    await trackReviewEvents("transaction_corrected", userId, orgId, {
      ...
      confidence_bucket: data.old_confidence
        ? data.old_confidence < 0.5
          ? "low"
          : data.old_confidence < 0.8
            ? "medium"
            : "high"
        : "none",
    }),

  transactionAccepted: async (
    userId: string,
    orgId: string,
    data: {
      transaction_id: string;
  ...
  ruleCreatedFromCorrection: async (
    userId: string,
    orgId: string,
    data: {
      rule_signature: string;
  ...
  tableVirtualizationPerformance: async (
    userId: string,
    orgId: string,
    data: {
      total_items: number;
