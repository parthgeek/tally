...
describe("Plaid OAuth Error Handling", () => {
  describe("getErrorMessage", () => {
    ...
    it("should return correct message for INSTITUTION_ERROR", () => {
      ...
      expect(message).toBe(
        "Your bank is experiencing technical difficulties. Please try again later."
      );
    });
  ...
  describe("validatePlaidConfig", () => {
    ...
    it("should throw when PLAID_CLIENT_ID is missing", () => {
      ...
      expect(() => validatePlaidConfig()).toThrow(
        "Missing required environment variables: PLAID_CLIENT_ID"
      );
    });
    ...
    it("should throw when multiple env vars are missing", () => {
      ...
      expect(() => validatePlaidConfig()).toThrow(
        "Missing required environment variables: PLAID_CLIENT_ID, PLAID_SECRET"
      );
    });
  });
});
