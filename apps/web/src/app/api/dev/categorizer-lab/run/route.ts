...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!isCategorizerLabEnabled()) {
    return NextResponse.json({ error: "Categorizer lab is not available" }, { status: 404 });
  }
  ...
  try {
    ...
    for (const labTx of dataset) {
      try {
        ...
        const result = mapCategorizationResultToLab(labTx.id, categorizationResult, engine, {
...
export async function GET(): Promise<NextResponse> {
  if (!isCategorizerLabEnabled()) {
    return NextResponse.json({ available: false, message: "Lab is disabled" }, { status: 404 });
  }
