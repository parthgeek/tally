...
- **T-6 (SHOULD)** Test the entire structure in one assertion if possible
  ...
  ```
- **T-7 (MUST)** Configure test runner with proper globals. For Vitest, use `globals: true` in config to avoid import/global conflicts.

---

### 4 — Error Handling

- **E-1 (MUST)** Use structured error responses with error codes for API routes
- **E-2 (SHOULD)** Implement proper error boundaries in React components  
- **E-3 (MUST)** Log errors with context using structured logging (PostHog/Sentry)

---

### 5 — Security & Authentication

- **SEC-1 (MUST)** Use `getUser()` instead of `getSession()` for authentication in server contexts
- **SEC-2 (MUST)** Implement proper CSRF protection for state-changing operations
- **SEC-3 (MUST)** Validate organization access for all multi-tenant operations
- **SEC-4 (MUST)** Validate all user inputs with Zod schemas
- **SEC-5 (SHOULD)** Implement proper rate limiting for API endpoints

---

### 6 — Modern React Patterns

- **R-1 (SHOULD)** Use React Query/TanStack Query for server state management
- **R-2 (SHOULD)** Prefer custom hooks for reusable stateful logic
- **R-3 (MUST)** Use proper TypeScript generic constraints for component props
- **R-4 (SHOULD)** Use React.memo() for expensive components that re-render frequently

---

### 7 — API Design

- **API-1 (MUST)** Use consistent HTTP status codes (200, 201, 400, 401, 403, 404, 500)
- **API-2 (MUST)** Implement proper pagination for list endpoints
- **API-3 (SHOULD)** Use OpenAPI/JSON Schema for API documentation
- **API-4 (MUST)** Use consistent error response format across all endpoints
...
- **S-4 (MUST)** Store sensitive API keys in Supabase Vault for database access. For Edge Functions, use encrypted environment variables with proper access controls.
